import type { TypedDocumentString } from 'types/generated/auth/graphql';
...
export function client({ adminSecret, endpoint }: HasuraAdapterClient) {
  if (!adminSecret) throw new TypeError('Hasura client error: Please provide an adminSecret');

  if (!endpoint) throw new TypeError('Hasura client error: Please provide a graphql endpoint');
