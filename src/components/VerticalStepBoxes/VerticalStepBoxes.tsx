...
const VerticalSteps = React.forwardRef<HTMLButtonElement, VerticalStepsProps>(
  (
    ...
    return (
      <nav aria-label="Progress" className="max-w-full">
              ...
              <li key={stepIdx} className="relative">
                <div className="flex w-full max-w-full items-center">
                  <button
                    ...
                    className={cn(
                      'group flex w-full cursor-pointer justify-center gap-4 rounded-large px-3 py-2.5',
                      stepClassName,
                    )}
                  ...
                  >
                        ...
                        <div className="relative">
                          ...
                          >
                            <div className="flex items-center justify-center">
                              {status === 'complete' ? (
                                <CheckIcon className="h-6 w-6 text-brand-gray-1000" />
                              ) : (
                                <span>{stepIdx + 1}</span>
                              )}
                            </div>
                    ...
                    <div className="flex-1 text-left">
                      <div>
                        <div
                          className={cn(
                            'mb-1 font-medium text-default-foreground transition-[color,opacity] duration-300 group-active:opacity-70 sm:text-lg',
                            {
                        ...
                        <div
                          className={cn(
                            'text-sm text-default-600 transition-[color,opacity] duration-300 group-active:opacity-70',
                            {
                              'text-default-500': status === 'inactive',
                            },
                          )}
                ...
                {stepIdx < steps.length - 1 && !hideProgressBars && (
                  <div
                    aria-hidden="true"
                    className={cn(
                      'pointer-events-none absolute left-3 top-[calc(64px_*_var(--idx)_+_1)] flex h-[calc(100%-22px)] -translate-y-[54%] items-center px-4',
                    )}
                    style={{
                      // @ts-ignore
                      '--idx': stepIdx,
